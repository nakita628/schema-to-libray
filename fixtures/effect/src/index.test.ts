...
describe('schemaToEffect fixtures', () => {
  it.each(fixtures)('%s', async (name) => {
    ...
    expect(await Effect.runPromise(fmt(schemaToEffect(input)))).toBe(expected)
  })
})

describe('schemaToEffect readonly fixtures', () => {
  it.each(READONLY_FIXTURES)('%s', async (name) => {
    const dir = join(fixturesDir, name)
    const input = JSON.parse(readFileSync(join(dir, 'input.json'), 'utf-8'))
    const expected = readFileSync(join(dir, 'output.ts'), 'utf-8')
    expect(await Effect.runPromise(fmt(schemaToEffect(input, { readonly: true })))).toBe(expected)
  })
})
...
describe('schemaToEffect split fixtures (parseSchemaFile + schemaToEffect)', () => {
  it.each(SPLIT_FIXTURES)('%s', async (name) => {
    ...
    expect(await Effect.runPromise(fmt(schemaToEffect(schema)))).toBe(expected)
  })
})
