...
function array(schema: JSONSchema, rootName: string, isEffect: boolean = false): string {
  ...
  if (typeof schema.minItems === 'number' && typeof schema.maxItems === 'number') {
    if (schema.minItems === schema.maxItems) {
      actions.push(`Schema.itemsCount(${schema.minItems})`)
    } else {
      actions.push(`Schema.minItems(${schema.minItems})`)
      actions.push(`Schema.maxItems(${schema.maxItems})`)
    }
  } else if (typeof schema.minItems === 'number') {
    actions.push(`Schema.minItems(${schema.minItems})`)
  } else if (typeof schema.maxItems === 'number') {
    actions.push(`Schema.maxItems(${schema.maxItems})`)
  }
