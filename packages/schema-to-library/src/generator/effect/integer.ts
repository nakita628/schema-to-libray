...
export function integer(schema: JSONSchema): string {
  if (schema.format === 'bigint') {
    const actions = [
      schema.minimum !== undefined ? `Schema.greaterThanOrEqualToBigInt(BigInt(${schema.minimum}))` : undefined,
      schema.maximum !== undefined ? `Schema.lessThanOrEqualToBigInt(BigInt(${schema.maximum}))` : undefined,
